  ...
  async.forEach(options.fields, function(path, callback) {
    ...
    switch (field.type) {
      ...
      case 'gcsimage':
        ...
        break;
      case 'gcsavatar':
        actionQueue.push(field.getRequestHandler(this.item, this.req, this.res, options.paths, function(err) {
          if (err && options.flashErrors) {
            this.req.flash('error', field.label + ' upload failed - ' + err.message);
          }
          progress(err);
      ...
      case 'text':
        // add custom maximum validation
        if (field.options.maxLength && data[field.path].length > field.options.maxLength) {
          message = field.options.maxLengthErr ? field.options.maxLengthErr : 'Max length: ' + field.options.maxLength;
          addValidationError(field.path, message);
          invalidated = true;
        }
        break;
