...
function signinWithUser(user, req, res, onSuccess) {
  ...
  req.session.regenerate(function() {
    ...
    if (keystone.get('cookie signin') && user.password) {
      ...
      // set user's avatar to cookie for keystone-plugin (chatroom)
      if (user.avatar) {
        var { filename, gcsBucket, gcsDir } = user.avatar;
        if (filename && gcsBucket && gcsDir) {
          var avatarURL = `https://storage.googleapis.com/${gcsBucket}/${gcsDir}${filename}`;
          res.cookie('keystone.avatar', avatarURL, { maxAge: cookieOpts.maxAge });
        }
      }
    }
    onSuccess(user);
...
exports.signout = function(req, res, next) {
  keystone.callHook(req.user, 'pre:signout', function(err) {
    ...
    res.clearCookie('keystone.avatar');
    req.user = null;
